...
rejectall = 0.5;% overall rejection rate, overall fraction of misclassified samples must be lower than this value
rejecttp = 0.5;% we also need to have a true positive rate that is higher than this value...
rejecttn = 0.3;% ...and a true negative rate that is higher than this value, which should generally be less than true postiive criterion because we do not want to miss too many infested patches ...
rejectdr = 0.7;% ... and finally we also need to have an overall precision that is higher than this value to make sure that we are not missing too many patches with pests
...
for ann = 1:200
    ...
    if ((truepos >= rejecttp) + (trueneg >= rejecttn) + (detectrate >= rejectdr) + (misclass_samples <= rejectall)) == 4% iff all the criteria are satisified
        % use the classification performance values from the new network as the new criteria for future ANN rejection
        rejectall = misclass_samples;
        rejecttp = truepos;
        rejecttn = trueneg;
        rejectdr = detectrate;
...
[Lc,Lcm] = confusion(new_output', ANN_output);% get the confusion matrix between ANN predictions and actual output
plotconfusion(new_output', ANN_output)% plot confusion matrix for the new data, to see how many true negatives etc we have in the new dataset with the retained ANN
